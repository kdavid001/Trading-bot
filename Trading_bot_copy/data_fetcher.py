...
class DataFetcher:
    ...
    def _get_intraday_data(self, symbol, interval, trading_type):
        """Fetch intraday data with improved symbol handling and fallback logic"""
        try:
            ...
            if self.td:
                ...
                # Format symbol for TwelveData: uppercase and no slash
                if '/' in symbol:
                    base, quote = symbol.split('/')
                    formatted_symbol = (base + quote).upper()
                else:
                ...
                try:
                    ...
                    if data.empty:
                        raise ValueError(f"TwelveData returned empty data for {formatted_symbol}")

                    return self._clean_data(data, trading_type)

                except Exception as e:
                    print(f"⚠️ TwelveData intraday fetch failed for {formatted_symbol}: {e}")

            # Fallback to Alpha Vantage for forex
            if '/' in symbol and self.av_fx:
                base, quote = symbol.split('/')
                ...
                try:
                    data, _ = self.av_fx.get_currency_exchange_intraday(
                        ...
                        outputsize='compact'  # compact for recent data to avoid timeout
                    )
                    df = pd.DataFrame(data).transpose()
                    if df.empty:
                        raise ValueError(f"Alpha Vantage returned empty data for {base}/{quote}")
                    return self._clean_data(df, trading_type)

                except Exception as e:
                    print(f"⚠️ Alpha Vantage intraday fetch failed for {base}/{quote}: {e}")

            # Avoid yfinance fallback for Forex intraday 15min because it doesn't provide this data reliably
            # For other symbols or intervals, fallback to yfinance for intraday
            if not ('/' in symbol and interval.lower() == '15min'):
                return self._get_yfinance_data(symbol, interval, 1, trading_type)  # 1 year max for intraday
            else:
                raise ValueError(f"No reliable intraday data source found for {symbol} at interval {interval}")

        except Exception as e:
            print(f"⚠️ Intraday fetch failed overall for {symbol} at {interval}: {e}")
            # Final fallback to yfinance but only if not 15min Forex pair (to avoid empty data)
            if not ('/' in symbol and interval.lower() == '15min'):
                return self._get_yfinance_data(symbol, interval, 1, trading_type)
            else:
                # Return empty dataframe to avoid crash
...
if __name__ == "__main__":
    ...
    print("'EUR/USD (5 years 15mins):")
    eur_data = fetcher._get_intraday_data('EUR/USD', interval='15min', trading_type='forex')

    print(f"Retrieved {len(eur_data)} daily bars")
    if not eur_data.empty:
        print(f"From {eur_data.index[0].date()} to {eur_data.index[-1].date()}")

    eur_data.to_csv("data/eur_data.csv", index=True)
    print("✅ Saved all data to eur_data.csv")
